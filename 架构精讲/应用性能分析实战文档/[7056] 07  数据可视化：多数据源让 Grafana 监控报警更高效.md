<p data-nodeid="41693" class="">这一讲我将带领你学习可视化监控套件 Grafana。Grafana 是一个开源的数据可视化的平台，所以它既不会监控应用，也不会产生监控数据，更不会对接原始数据进行分析存储。<strong data-nodeid="41699">它仅专注数据可视化本身</strong>。</p>
